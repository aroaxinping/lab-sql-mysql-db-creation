...
-- ids 5 and 6 share the same VIN on purpose -- this duplicate is exactly what
-- the bonus challenge asks us to find and clean up later.
...
-- the lab's sample table shows "-" for email -- that's "no data", not a literal
-- string, so it maps to NULL rather than the text "-"
...
-- fixed the lab's "Mimia" typo for salesperson 5's store to "Miami"

-- car_id / customer_id / salesperson_id below reference each table's own
-- auto-increment id, in the same 1-based order the rows were just inserted in
-- (so invoice car=1 -> the Volkswagen Tiguan, customer=1 -> Pablo Picasso, etc.)
